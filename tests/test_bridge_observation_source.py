...
def test_repeated_observes_pull_payloads_in_exact_order() -> None:
    payload_source = RecordingBridgePayloadSource(
        bridge_payload(screenshot_id="shot-1"),
        bridge_payload(screenshot_id="shot-2"),
    )
    source = source_for(payload_source)
...
def test_bridge_payloads_drive_existing_skill_runner_with_dry_run_input() -> None:
    payload_source = RecordingBridgePayloadSource(
        bridge_payload(
            ui_state="dialogue",
